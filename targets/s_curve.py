...
class SCurve(GrayCodedTarget):
    """SCurve.

    SCurve, discretised onto a grid,
    ...
    def _sample_continuous(self, n: int) -> torch.Tensor:
        """Sample from the continuous SCurve.
        ...
        """
        samples, _ = datasets.make_s_curve(n, noise=self.std)
        samples = samples[:, [0, 2]] * self.scale / 5.5
    ...
    def visualise(self, x: torch.Tensor, **kwargs) -> dict[str, Image.Image]:
        """Visualise the discretised SCurve.
